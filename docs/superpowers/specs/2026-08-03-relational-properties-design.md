...
- **New client → old client.** The pull path writes values verbatim to frontmatter and `setNoteProperties`; the old build's `inferPropertyType` yields `text` for the array, so it renders raw JSON. Ugly, lossless, non-crashing, and values round-trip unmodified back through that client's pushes.
  _Verification item before release:_ confirm a currently-released build renders an array-valued property without throwing. `multiselect` already renders arrays, so the path exists — but this must be checked against a real released build, not assumed.
