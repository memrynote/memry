...
- **New client → old client. Passive sync is lossless; the first property edit on that device is not.** Established by inspecting the shipped release `v2026-07-19.2`:
  - _Pull_ assigns the value verbatim into frontmatter (`note-handler.ts`, `parsed.frontmatter.properties = remoteProperties`), and _push_ re-reads it from the file with `extractProperties(frontmatter)` (`note-sync.ts`). The index-DB copy never reaches the wire, so a note that is only synced — never edited — round-trips byte-identical.
  - _Editing any property on that note_ does not. `use-properties.ts` rebuilds the entire record from index-DB values, where `deserializeValue(..., 'text')` has already flattened the array into a JSON string. That string is written to YAML and pushed. The relation is gone: quoted string in the vault file, no `property_refs`, no graph edge, no backlink. Silent and permanent.
...
- **Type inference on the empty default.** `getDefaultValueForType('relation')` is `[]` and no type crosses the `properties:set` boundary, so the first thing main indexes for a new relation is an empty array — and `isRelationValue([])` is false by construction. Inference alone therefore cannot type a relation on the device that created it. `getPropertyType` (`property-queries.ts`) resolves this with a structural override: an array of `memry://` URIs is a relation whatever the stored definition says. Read-time only — the definitions table is a derived cache of `.memry/properties.md`, and `PropertyDefinitionSchema` has no `relation` member, so persisting one would make the whole file fail to parse and drop every definition in it.
