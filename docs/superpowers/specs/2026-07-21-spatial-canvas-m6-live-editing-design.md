...
5. **Yjs activation boundary (Task 5 discovery).** `ContentArea` gates `useYjsCollaboration` on `syncActive` (`ContentArea.tsx:1336-1338`), which is only true for sync status `idle`/`syncing`/`offline` — reached only when **authenticated** (`sync-context.tsx:227`). So the R17 shared-Y.Doc path is exercised **only for authenticated/synced users**. Unauthenticated/local users (and the E2E vault) edit notes via non-collaborative BlockNote + `onMarkdownChange` markdown-save, exactly like `note.tsx`; `onMarkdownChange` is gated behind `!yjsFragment` (`use-editor-sync.ts:304`), so there is no double-write when Yjs is active. `<EmbeddedNoteEditor>` correctly mirrors `note.tsx` for both modes.
   - **R17 validation boundary (Kaan decision):** the Yjs two-editor sharing/teardown is validated by the **Tasks 3-4 registry unit tests**. The `#19` E2E runs unauthenticated, so it validates the **non-collaborative no-duplicate** case, not the Yjs shared-doc path. This coverage split is intentional (authenticated-sync E2E fixtures are out of M6 scope).
6. **Unauthenticated tab+card co-edit clobber — M7 hardening item (Kaan decision: document, harden before default-on).** For _unauthenticated_ users, the same note open in a tab AND active on a canvas card = two independent non-collaborative markdown editors; concurrent edits in both surfaces can clobber each other (last-save-wins; lost edits, not duplicate blocks). Authenticated users are safe (shared Y.Doc). Pre-M6 this was unreachable (one tab per note); M6's card makes it reachable. Accepted as a known limitation of the opt-in (default-OFF) phase; **must be addressed before M7 default-on** (e.g. an unauthenticated-only "already open in a tab → read-only card + edit-in-tab hint" guard). Same-envelope facet (verify with the same M7 guard): because `isSideEffectOwner` defaults `true` when collaboration is disabled (unauthenticated), two such editors coexisting in **split view** would each run `ContentArea`'s task auto-conversion → possible **duplicate task creation** (not only body clobber).
7. **Restore-wipe on in-session remount (Task 5 bug — fixed).** Reopening a canvas that already contained cards (tab switch away+back, or close+reopen) rendered zero cards. Root cause was **not** the overlay: on an in-session unmount React tears down the `<Excalidraw>` child _before_ `canvas-editor.tsx`'s effect cleanup runs its `persister.flush()`. By then `apiRef.current.getSceneElements()` returns `[]`, so the flush serialized an **empty** scene and — because `empty !== lastSaved` — persisted it over the real scene, wiping every card rectangle. The next mount loaded that empty `initialScene`. Instrumented evidence (temporary): the unmount flush logged `serialize els:0 → save els:0`, and `canvas.get(id)` returned 0 elements post-remount. `page.reload()` was safe because the save-registry `beforeunload`/`app:request-flush` flush runs while Excalidraw is still alive — so no earlier E2E, all of which used reload or never reopened, caught it. **Fix:** the persister's `serialize` contract already promised `null` "when the scene is … already torn down"; the impl only checked `!api`. It now also treats a **disconnected wrapper** (`!wrapperRef.current?.isConnected`) as torn-down and returns `null`, so the unmount flush no longer overwrites the scene. Normal saves and the `beforeunload`/quit flush keep the wrapper connected and persist as before. Regression: `canvas-editing.e2e.ts` "cards survive an in-session canvas remount".
