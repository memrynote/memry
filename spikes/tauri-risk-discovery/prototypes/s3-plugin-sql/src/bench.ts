// S3 Option B bench harness — exercises plugin-sql via renderer-side helpers.
...
export const OPTION = 'B' as const
...
function makeTests(): BenchTest[] {
  return [
    ...
    },
    {
      ...
      runs: 1,
      fn: async (i) => {
        await bulkInsertNotes(buildBulkBatch(`bulk-B-${i}`))
      },
    },
    {
...
export async function runAllBench(onProgress: (msg: string) => void): Promise<TestRun[]> {
  const log = (m: string) => {
    onProgress(m)
    if (DIAGNOSTIC_LOG) console.log(`[bench-B] ${m}`)
  }
  log(`seeding ${SEED_NOTES} notes`)
  const t0 = performance.now()
  await seedNotes(SEED_NOTES)
  log(`notes seeded in ${(performance.now() - t0).toFixed(0)}ms`)
  log(`seeding ${SEED_EMBEDDINGS} embeddings`)
  const t1 = performance.now()
  await seedEmbeddings(SEED_EMBEDDINGS)
  log(`embeddings seeded in ${(performance.now() - t1).toFixed(0)}ms`)
