...
- [x] T066 [US2] Offline matrix automation: airplane-mode edit + create → force-quit → relaunch → reconnect → complete sync; ≥ 20 scripted runs, 100% pass — `apps/mobile/.maestro/us2-offline-matrix.yaml` + driver script _(AUTOMATION LANDED, RUN OUTSTANDING — flow + `scripts/us2-offline-matrix.mjs` (`pnpm --filter @memry/mobile test:offline-matrix -- --runs 20`). The radio is NOT scriptable from Maestro (neither the simulator nor a device exposes airplane mode to the accessibility tree), so the driver owns the transitions and the scenario is TWO flows with the cut between them — one file would mean restoring the network only after the flow exited, making the reconnect assertion pass vacuously offline. The cut comes from the APP: a dev-build-only switch backed by a marker FILE the driver writes into the app's document container, making the HTTP adapter report offline and reject every request — a file rather than a deep link because the dev-client shell swallows the `memry://` scheme (tried, does not work), and it survives the force-quit the scenario depends on. `simctl status_bar --dataNetwork hide` only REPAINTS the status bar and leaves the simulator online, so driving the matrix with it would report a green that means nothing; the offline flow asserts the app's own Offline banner for the same reason. Simulator runs are unattended; `--device` prompts, since hardware has no scriptable radio. **DONE 2026-08-27: 20/20 on the iPhone 17 Pro simulator against staging, verified from the Maestro artefacts rather than the summary line — 20 `us2-offline-matrix` and 20 `us2-offline-reconnect` runs, no FAILED command in any of them.** Getting there took three product fixes the flow surfaced and one environment fix: the sync banner read NetInfo and never the dev switch, so it reported "Sending N changes…" while nothing could send; nothing on screen was addressable by `id:` because a View wrapping Text is not an accessibility element on iOS; note creation awaited a full list reload before navigating, which on a seeding vault queued behind the bootstrap on the one SQLite connection; and expo-dev-menu, whose onboarding a freshly erased simulator has never finished, took the screen the first time the app presented a second window (`quietDevMenu()` in the driver). The reconnect half then caught the real one: **the outbox was never drained on the online transition** — 122 rows with `attempt_count` still 0. The flow waits on the OUTBOX-depth banner (`Unsynced changes`), deliberately distinct from the pull indicator — a run that waited on "Syncing" would call a still-full outbox a success. The runs are simulator runs driven by the app-level switch, which is `__DEV__`-only; hardware confirmation of the same scenario lives in T077's on-device convergence gate, not here.)_
