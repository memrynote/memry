...
- [x] N600 Tappable `#tag` plus the tag screen it needs; `NoteBlockView.swift` marks tags deliberately unlinked today because there is nowhere to go
- [x] N601 `dateMention`: the date picker, `remindMe`, and the date suggestions desktop offers
- [x] N602 Wiki link menu: note search, heading selection, `displayAs` alias, embed versus link, and creating a note from a broken link
- [x] N603 Paste-link menu: url, mention, embedded video, bookmark
...
- [x] N605 [P] Tappable `inlineCheckbox` inside a table cell
...
- [x] R02 Edit a table's cell text, a cell colour and a column width on iOS, and confirm desktop regenerates `<!-- table-layout:… -->` and `<!-- table-colors:… -->` with the same bytes it would have written itself — closed by adding `setCellText`/`setCellProp` cases to `block-edit.json` and running desktop's real `yDocToMarkdown` over the documents they pin
...
- [x] R06 Upload a picture from iOS, open the note on desktop, confirm the file lands in the vault with a matching checksum, then delete it from iOS and confirm the chunks are dereferenced — the checksum round-trip is pinned by `attachment_upload`'s writer/reader test and the cross-port `attachment-manifest` class; the wire behaviour (what `initiate` declares, raw chunk bytes, and §14.8's dereference on delete) by `crates/memry-core/tests/attachment_upload_wire.rs`
- [x] R07 Open a note whose attachment bytes have not arrived, confirm a placeholder rather than a gap, and confirm the picture appears on arrival without the note being recreated — `apps/ios/MemryTests/NoteAttachmentTests.swift`; `.waitingFor` renders `AttachmentPlaceholder`, and the late arrival rebinds on the same view model
