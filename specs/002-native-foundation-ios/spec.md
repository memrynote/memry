...
- **FR-040**: The editor MUST render and preserve every type in the shared schema's registry. The registry comprises: default block types `audio`, `bulletListItem`, `checkListItem`, `codeBlock`, `divider`, `file`, `heading`, `image`, `numberedListItem`, `paragraph`, `quote`, `table`, `toggleListItem`, `video`; Memry block types `taskBlock`, `callout`, `file`, `youtubeEmbed`, `bookmark`, `toggleListItem`, `mathBlock`, `diagram`, `whiteboard`, where `file` and `toggleListItem` are Memry specifications overriding the defaults of the same name, `diagram` is a Mermaid diagram stored as a ` ```mermaid ` fence, and `whiteboard` is a reference to a canvas stored as a `![whiteboard](memry://canvas/<id>)` line; inline content types `text`, `link`, `wikiLink`, `linkMention`, `hashTag`, `dateMention`, `inlineImage`, `inlineCheckbox`; and styles `bold`, `italic`, `underline`, `strike`, `code`, `textColor`, `backgroundColor`. Required behaviour: every listed type renders and is preserved on save; editing is full for text-bearing types; and `file`, `audio`, `video`, `bookmark`, `youtubeEmbed`, and `mathBlock` blocks render in place with their existing data but are not creatable on the phone in this feature (`mathBlock` renders as its LaTeX source, not as typeset mathematics: KaTeX's JavaScript and fonts are bytes the phone's WebView refuses for the same reason it refuses shiki). `diagram` behaves the same way and for the same reason: the phone renders its Mermaid SOURCE, labelled and editable, rather than the drawing, because the renderer is ~3 MB. `whiteboard` renders as a labelled card and is not creatable on the phone: the drawing lives in a canvas, and canvases are absent from this feature. The enumerated list MUST be derived from the shared schema's type registry, and a test MUST fail when a type exists in the registry but not in this requirement.
