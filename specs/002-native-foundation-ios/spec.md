...
- **FR-040**: The editor MUST render and preserve every type in the shared schema's registry. The registry comprises: default block types `audio`, `bulletListItem`, `checkListItem`, `codeBlock`, `divider`, `file`, `heading`, `image`, `numberedListItem`, `paragraph`, `quote`, `table`, `toggleListItem`, `video`; Memry block types `taskBlock`, `callout`, `file`, `youtubeEmbed`, `bookmark`, `toggleListItem`, `mathBlock`, `diagram`, where `file` and `toggleListItem` are Memry specifications overriding the defaults of the same name and `diagram` is a Mermaid diagram stored as a ` ```mermaid ` fence; inline content types `text`, `link`, `wikiLink`, `linkMention`, `hashTag`, `dateMention`, `inlineImage`, `inlineCheckbox`; and styles `bold`, `italic`, `underline`, `strike`, `code`, `textColor`, `backgroundColor`. Required behaviour: every listed type renders and is preserved on save; editing is full for text-bearing types; and `file`, `audio`, `video`, `bookmark`, `youtubeEmbed`, and `mathBlock` blocks render in place with their existing data but are not creatable on the phone in this feature (`mathBlock` renders as its LaTeX source, not as typeset mathematics: KaTeX's JavaScript and fonts are bytes the phone's WebView refuses for the same reason it refuses shiki). `diagram` behaves the same way and for the same reason: the phone renders its Mermaid SOURCE, labelled and editable, rather than the drawing, because the renderer is ~3 MB. The enumerated list MUST be derived from the shared schema's type registry, and a test MUST fail when a type exists in the registry but not in this requirement.
...
| Attachments                                | Partial                                              | `file`, `audio`, and `video` blocks render their existing metadata in place; downloading their bytes is deferred and uploading any attachment from the phone is deferred. Inline images are the exception: their bytes download lazily on unmetered connections with a per-item override (FR-045)                                                                                                                                                                                                             |
