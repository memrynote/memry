...
- [ ] T232 [US2] Implement the **packed CRDT envelope** of `docs/protocol/04` §4.11 in `crates/memry-core/src/protocol/envelope.rs`: `pack` and `unpack` over the 160-byte header with the signature at offset 96 and the AEAD ciphertext following, and extend `crates/memry-core/tests/vectors.rs` with the committed `crdt-update` class. Discovered during T109 to T111: the class pins encryption, not CRDT semantics, so it belongs beside the record envelope and no existing Phase 3 task assigned it. **T089 cannot be ticked until this lands.**
