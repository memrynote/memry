...
/** The one global a host must provide to receive guest messages. */
type HostTransport = { postMessage(value: string): void }

/**
 * Resolve the host transport.
 *
 * `MemryHost` is the name every host injects from now on — the Swift shell has
 * no React Native in it, and a global called `ReactNativeWebView` would be a
 * lie about what is on the other end. The legacy name is still accepted so the
 * frozen RN shell keeps working against a rebuilt asset: this is a transport
 * rename, not a protocol change, and `BRIDGE_PROTOCOL_VERSION` stays 1.
 */
function resolveHostTransport(): HostTransport | undefined {
  const globals = globalThis as {
    MemryHost?: HostTransport
    ReactNativeWebView?: HostTransport
  }
  return globals.MemryHost ?? globals.ReactNativeWebView
}
...
export function createGuestBridge(): GuestBridge {
  const host = resolveHostTransport()
  const bridge = new GuestBridge((payload) => host?.postMessage(payload))
