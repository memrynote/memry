...
// Returns a parsed result with date and optional time. English only (spec 004
// D1). `now` is the caller's clock: the parser never reads one.
 ...
 */
const getNextDayOfWeek = (dayIndex: number, startFrom: Date): Date => {
 ...
 */
const getPreviousDayOfWeek = (dayIndex: number, startFrom: Date): Date => {
  const today = startOfDay(startFrom)
  const currentDay = today.getDay()
 ...
 */
export const parseNaturalDate = (input: string, now: Date): NaturalDateParseResult => {
