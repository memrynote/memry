 ...
 * detail dialog.
 *
 * English only (spec 004 D1) — a phrase in any other language simply does not
 * parse and stays part of the task title, which is the safe failure. `now`
 * stamps the config's `createdAt`; nothing here reads the clock.
 */

import { addDays, startOfDay } from './dates.ts'
import type { RepeatConfig } from './types.ts'
...
const baseConfig = (now: Date): Omit<RepeatConfig, 'frequency' | 'interval'> => ({
  endType: 'never',
  completedCount: 0,
...
export const parseRepeatPhrase = (phrase: string, anchor: Date, now: Date): RepeatConfig | null => {
  ...
  if (rest === 'weekday' || rest === 'weekdays') {
    return { ...baseConfig(now), frequency: 'weekly', interval: 1, daysOfWeek: WEEKDAYS }
  }
  if (rest === 'weekend' || rest === 'weekends') {
    return { ...baseConfig(now), frequency: 'weekly', interval: 1, daysOfWeek: WEEKEND }
  }
  ...
  if (unitMatch) {
    ...
    switch (unitMatch[2]) {
      case 'day':
        return { ...baseConfig(now), frequency: 'daily', interval }
      case 'week':
        return { ...baseConfig(now), frequency: 'weekly', interval }
      case 'month':
        return {
          ...baseConfig(now),
          frequency: 'monthly',
      ...
      default:
        return { ...baseConfig(now), frequency: 'yearly', interval }
    }
  }
  ...
  if (dayListMatch) {
    ...
    if (interval !== null && daysOfWeek !== null) {
      return { ...baseConfig(now), frequency: 'weekly', interval, daysOfWeek }
    }
  }
...
export const findRepeatPhrase = (
  ...
  for (const every of input.matchAll(/\bevery\b/gi)) {
    ...
    for (let count = wordEnds.length; count >= 2; count--) {
      ...
      const config = parseRepeatPhrase(text, anchor, now)
 ...
 */
export const firstOccurrenceFor = (config: RepeatConfig, from: Date): Date => {
