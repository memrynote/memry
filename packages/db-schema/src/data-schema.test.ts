...
describe('data-schema exports', () => {
  it('re-exports sync foundation tables', () => {
    ...
    expect(dataSchema.noteMetadata).toBeDefined()
    expect(dataSchema.propertyDefinitions).toBeDefined()
    ...
    expect(dataSchema.noteMetadata[Symbol.for('drizzle:Name')]).toBe('note_metadata')
    expect(dataSchema.propertyDefinitions[Symbol.for('drizzle:Name')]).toBe('property_definitions')
  })
})
