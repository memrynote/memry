...
describe('mapFiles', () => {
  ...
  it('maps a root-level md file to Markdown folder', () => {
    const plan = mapFiles([{ relPath: 'note.md', absPath: '/root/note.md' }])
  ...
  it('maps a nested file to a Markdown sub-folder', () => {
    const plan = mapFiles([{ relPath: 'work/project.md', absPath: '/root/work/project.md' }])
    expect(plan.notes[0].vaultFolder).toBe('Markdown/work')
    expect(plan.notes[0].title).toBe('project')
  })

  it('handles .markdown extension', () => {
    const plan = mapFiles([{ relPath: 'note.markdown', absPath: '/root/note.markdown' }])
    expect(plan.notes).toHaveLength(1)
    expect(plan.notes[0].title).toBe('note')
  })
  ...
  it('skips non-markdown files', () => {
    const plan = mapFiles([
      { relPath: 'note.md', absPath: '/root/note.md' },
      { relPath: 'image.png', absPath: '/root/image.png' },
      { relPath: 'doc.pdf', absPath: '/root/doc.pdf' }
    ])
    expect(plan.notes).toHaveLength(1)
    expect(plan.notes[0].title).toBe('note')
  })
  ...
  it('maps deeply nested file', () => {
    const plan = mapFiles([{ relPath: 'a/b/c/deep.md', absPath: '/root/a/b/c/deep.md' }])
  ...
  it('strips only the outermost extension from filename', () => {
    const plan = mapFiles([{ relPath: 'my.note.md', absPath: '/root/my.note.md' }])
