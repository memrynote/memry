...
Attachment commands write into the vault attachment folder. `import-files` copies supported markdown, PDF, image, audio, and video files into the target folder; imported markdown is indexed immediately for follow-up CLI commands. `notes attach` copies the file in, appends the same embed the desktop app writes (an inline image, or a clickable file block for documents), and queues the upload in the shared sync outbox — the desktop app drains that queue on its next sync start, so the attachment reaches other devices without any extra step. `notes move` re-points relative attachment embeds inside the moved note's body, matching the desktop app.
