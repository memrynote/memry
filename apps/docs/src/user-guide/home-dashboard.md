...
The Project widget is a chosen project in miniature. Its header carries the project's own name and icon, so several of them side by side stay tellable apart, and its body has the same five tabs as the project page: **Overview**, **Tasks**, **Notes**, **Files** and **Events**. Rows behave as they do on the page — open a note, tick a task off, change a status or a priority.

Pick the project from **Configure** in the widget menu. Two actions hand off to the full project page rather than happening in the widget: adding a task and adding a file both need the page's capture bar and its drop zone, so they open the project on that tab. The tab you are looking at is not remembered — a project widget opens on **Overview** each time Memry starts. If the project is later deleted or archived (including on another device, since boards sync) the widget says so and stays where it is; it never removes itself from your board.
