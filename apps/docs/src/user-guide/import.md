...
Import a [NotePlan 3](https://noteplan.co) vault: daily notes become real journal entries, other notes become notes, and NotePlan tasks become real Memry task rows.

1. Open **Settings → Import** and click **Import** next to **NotePlan**.
2. Choose the NotePlan folder — either the app's data folder (the picker opens there by default on macOS) or one of its `Backups/<stamp>` copies, which have the same shape. A bare `Notes/` folder also works.

| NotePlan                                                                                     | memrynote                                           |
| -------------------------------------------------------------------------------------------- | --------------------------------------------------- |
| `Calendar/YYYYMMDD.txt` daily note                                                           | Journal entry for that date                         |
| `Calendar/YYYY-Wnn` / `YYYY-MM` / `YYYY-Qn` / `YYYY`                                         | Note under `NotePlan/Calendar/`                     |
| `Notes/**`                                                                                   | Note under `NotePlan/<original folder tree>`        |
| `@Archive/**`                                                                                | Note under `NotePlan/Archive/`                      |
| First `# H1` line                                                                            | Note title (so `[[Wiki Links]]` to it resolve)      |
| `* task`, `* [x]` done, `* [-]` cancelled, `* [>]` scheduled                                 | Task row: open, completed, archived, open           |
| `>YYYY-MM-DD` on a task                                                                      | Task due date                                       |
| `@done(YYYY-MM-DD)` on a task                                                                | Task completed date                                 |
| `+ checklist`                                                                                | Plain `- [ ]` checkbox (not promoted to a task row) |
| `- bullet`                                                                                   | Plain bullet                                        |
| Hierarchical `#tag/sub`                                                                      | Note tags                                           |
| Frontmatter (`type`, `status`, `owner`, `team`, `date`, `attendees`, `created`, `folder`, …) | Note properties                                     |

Tasks land in your **Inbox** project, linked back to the note or journal entry they came from.
