...
- **Font Size** — A `−` / `+` stepper from 12px to 24px in 1px steps, default 16px, with the current size shown between the two buttons. Sets the base interface size; the whole app and note editor text scale with it, live on every step. The reset button next to the stepper puts it back to 16px. It is a stepper rather than a slider for the same reason Zoom is, below: resizing the text reflows the settings page under the pointer. Older versions of the app offered Small / Medium / Large only; those land on 14px, 16px and 20px respectively, and a device still on an older version follows your choice to the nearest of the three.
