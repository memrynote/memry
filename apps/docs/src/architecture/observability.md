...
What was left over was a residual `WindowError` bucket: no error object, a message naming no error
class, and a synthesized frame pointing at `…/out/renderer/index.html:0:0` — an event with no
usable payload at all, and no way to tell "the message named no error class" from "there was no
message". Two further codes split it, without copying any message text:

- `WindowError_ScriptError` / `WindowError_ResizeObserverLoop` — the message matched one of the
  engine's own fixed notification strings exactly. These literals are generated by Chromium and
  embed nothing from the document or the vault, so recognizing one is safe in a way that copying
  arbitrary message text is not. The text is still not carried onto the reported error.
- `WindowError_Empty` — the event carried no message at all.

Anything unrecognized still reports `WindowError`, so existing queries keep matching; it now means
"a message existed and named no error class" rather than "unknown".
...
Release builds shipped no sourcemaps and nothing uploaded any, so a renderer stack frame in
PostHog was a bundle hash and a line number. Error Tracking reported `resolved: true` while
`resolved_name` still equalled `mangled_name` — it had processed the frame and had no map to
apply. Locating a production `TypeError` meant downloading the released artifact, unpacking
`app.asar`, and reading `out/renderer/assets/index-<hash>.js` at the reported line by hand.

The renderer now builds with `build.sourcemap: 'hidden'` (`apps/desktop/electron.vite.config.ts`).
`hidden` emits the `.map` files but appends no `//# sourceMappingURL` comment, so the shipped
bundles are unchanged by the setting itself. The maps are **never packaged**:
`config/electron-builder.yml` excludes `out/**/*.map`, alongside the existing
`node_modules/**/*.map` rule.

`apps/desktop/scripts/upload-sourcemaps.mjs` runs in the release workflow between
`build:release` and `build-packaged-app.js`, once per platform job (each platform produces its own
content hashes, so each needs its own upload). It reads `POSTHOG_CLI_API_KEY` and
`POSTHOG_CLI_PROJECT_ID`; **with either unset it logs a skip and exits 0**, so a release cut before
those secrets exist is byte-for-byte what it is today. When configured it runs:

- `posthog-cli sourcemap inject` — writes a `//# chunkId=…` comment into the built JS. This is the
  one change the feature makes to what ships, and it has to happen before packaging, because that
  id is what ties a runtime frame back to an uploaded map.
- `posthog-cli sourcemap upload` — sends the maps under release `memry-desktop` at the app version.

The pass condition is a subsequent `window_error` arriving with `resolved_name` different from
`mangled_name`. Until that is observed, symbolication is configured but not proven.
