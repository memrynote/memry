...
#[tokio::test]
async fn set_local_only_flips_metadata_and_count() {
    ...
    let response = notes_set_local_only_inner(&conn, &created.id, true).unwrap();
    ...
    let response = notes_set_local_only_inner(&conn, &created.id, false).unwrap();
    assert!(response.success);
    let row = note_metadata::get_by_id(&conn, &created.id)
        .unwrap()
        .unwrap();
...
async fn local_only_count_excludes_soft_deleted_notes() {
    ...
    notes_set_local_only_inner(&conn, &kept.id, true).unwrap();
    notes_set_local_only_inner(&conn, &trashed.id, true).unwrap();
