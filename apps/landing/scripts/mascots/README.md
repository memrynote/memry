...
How to create a new hand-drawn mascot icon that matches the existing set and wire it into the
landing site. Written so a fresh session (human or agent) can go from "make me a _thoughts_
icon" to a merged PNG without rediscovering anything.

## What exists

- **Files:** `apps/landing/public/mascots/*.png`. `all.png` is the original 1024×1024 source
  sheet — never delete it.
- **Component:** `src/components/ui/mascot.tsx` renders the PNG. The site is light-only, so
  there is one variant per icon.
...
| Interiors  | transparent (paper shows through); do not fill with white                                        |
...
1. **QA contact sheet:** paste the new icon between 2–3 real ones (e.g. `calendar.png`,
   `lock.png`) on a cream `#faf1e6` row; also render a 4× NEAREST zoom.
   Check: stroke weight matches, one orange accent, nothing kisses an outline, reads at 32 px.
2. **Wire up:** chips/cards use `<Mascot src="/mascots/<name>.png" className="h-8 w-8" />`;
   dropdown items in `lib/constants.ts` use `{ icon: '/mascots/<name>.png', iconType: 'image' }`.
3. **Verify:** `pnpm --filter ./apps/landing typecheck`, then dev server + Playwright
...
The filename is the public URL (`/mascots/<name>.png`).
