...
const SEED_ITEMS: InboxItem[] = [
  ...
  { id: 4, type: 'note', title: 'Ask Mia about the cabin weekend', time: '1d', day: 'yesterday' },
  {
    id: 5,
    type: 'link',
    title: 'the-marginalian.org/attention',
    domain: 'the-marginalian.org',
    time: '1d',
    day: 'yesterday'
  },
  { id: 6, type: 'voice', title: 'Standup notes', duration: '1:05', time: '1d', day: 'yesterday' }
]
...
/* ── Item row — checkbox · type glyph · title · meta that swaps to snooze/archive on hover ── */
...
interface ItemRowProps {
  ...
  onRemove: (item: InboxItem, message: string) => void
}

function ItemRow({ item, isSelected, hasSelection, onToggleSelect, onRemove }: ItemRowProps) {
  return (
    <motion.li
      layout
      ...
      className={cn(
        ...
        isSelected ? 'bg-terracotta/5 ring-1 ring-inset ring-terracotta/25' : 'hover:bg-paper-alt'
      )}
    >
      ...
      <span className="relative flex min-w-[3.75rem] shrink-0 items-center justify-end">
        ...
        >
          <button
            type="button"
            onClick={() => onRemove(item, 'Snoozed until tomorrow')}
          ...
          <button
            type="button"
            onClick={() => onRemove(item, 'Item archived')}
...
export function InboxWidget({ className }: { className?: string }) {
  ...
  const todayItems = items.filter((item) => item.day !== 'yesterday')
  const yesterdayItems = items.filter((item) => item.day === 'yesterday')
  ...
  return (
    ...
    >
      ...
      >
        ...
        <span className="text-[11px] tabular-nums text-muted/50">{todayItems.length}</span>
      </button>
      ...
      {/* "Yesterday" section — same interactive rows, quieter day group */}
      <button
        type="button"
        onClick={() => setYesterdayOpen((prev) => !prev)}
      ...
      >
        ...
        />
        <span className="text-xs font-semibold tracking-[0.02em] text-muted">Yesterday</span>
        <span className="text-[11px] tabular-nums text-muted/50">{yesterdayItems.length}</span>
      </button>
      {yesterdayOpen && (
        <ul className="space-y-px">
          <AnimatePresence initial={false}>
          ...
          {yesterdayItems.length === 0 && (
            <li className="px-2 py-2 text-center text-[11px] text-muted/70">Cleared yesterday</li>
          )}
        </ul>
      )}
