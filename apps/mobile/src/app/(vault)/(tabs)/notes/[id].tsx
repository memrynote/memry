...
export default function NoteScreen() {
  ...
  /**
   * How far the header has been pushed off the top, never more than its own
   * height.
   *
   * `diffClamp` rather than a plain clamp so the header comes back on the way
   * UP from wherever the reader is, which is what makes it feel attached to the
   * gesture instead of to the top of the document.
   */
  const headerOffset = useMemo(
    () =>
      headerHeight > 0 ? Animated.multiply(Animated.diffClamp(scrollY, 0, headerHeight), -1) : null,
        ...
        chrome={
          <Animated.View
            ...
            style={[
              ...
              headerOffset ? { transform: [{ translateY: headerOffset }] } : null
            ]}
          >
