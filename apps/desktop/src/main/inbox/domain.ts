...
import { createInboxCrudHandlers, type InboxCrudHandlers } from '../ipc/inbox-crud-handlers'
import { createInboxBatchHandlers, type InboxBatchHandlers } from '../ipc/inbox-batch-handlers'
import { createInboxQueryHandlers, type InboxQueryHandlers } from '../ipc/inbox-query-handlers'
...
async function captureTextItem(input: CaptureTextInput): Promise<InboxCaptureResponse> {
  try {
    ...
    return { success: true, item: emitCapturedAndSync(db, row, tags) }
  } catch (error) {
    return {
      success: false,
      item: null,
...
function getInboxItem(itemId: string): ContractInboxItem | null {
  const db = requireDatabase()
