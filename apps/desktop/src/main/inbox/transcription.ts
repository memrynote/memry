...
import type { VoiceTranscriptionSettings } from '@memry/contracts/settings-schemas'
...
function isAutoGeneratedVoiceMemoTitle(title: string): boolean {
  return /^Voice memo(?: \(\d+:\d{2}\))?$/.test(title)
}

function titleFromTranscription(transcription: string): string | undefined {
...
function titleFromTimestamp(createdAt: string): string {
  ...
  return `Voice memo ${year}-${month}-${day} ${hours}:${minutes}`
}

function resolveVoiceMemoTitle(
  currentItem: { title: string; createdAt: string } | undefined,
  transcription: string,
  memoNameMode: VoiceTranscriptionSettings['memoNameMode']
): string | undefined {
  if (!currentItem || !isAutoGeneratedVoiceMemoTitle(currentItem.title)) {
    return undefined
  }

  if (memoNameMode === 'none') {
    return undefined
  }

  if (memoNameMode === 'timestamp') {
    return titleFromTimestamp(currentItem.createdAt)
  }

  return titleFromTranscription(transcription)
}
...
export async function transcribeAudio(
  ...
  try {
    ...
    const currentItem = db
      .select({ title: inboxItems.title, createdAt: inboxItems.createdAt })
      .from(inboxItems)
      .where(eq(inboxItems.id, itemId))
      .get()
    const autoTitle = resolveVoiceMemoTitle(currentItem, transcription, settings.memoNameMode)
