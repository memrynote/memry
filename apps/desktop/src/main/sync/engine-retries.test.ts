...
describe('SyncEngine', () => {
  ...
  // #2421: a reconnect pulls the change feed, which re-serves every body above
  // LAST_CURSOR; the per-note catch-up of open docs is gone.
  describe('#given WS reconnect #when handleWsConnected fires', () => {
    it('#then schedules the pull and pulls no open CRDT note by itself', async () => {
      ...
      const getOpenNoteIds = vi.fn(({ active = false } = {}) =>
        active ? ['editor-1'] : ['editor-1', 'note-1', 'note-2']
      )
      const deps = createMockDeps(getDb(), {
        crdtProvider: {
      ...
      const engine = new SyncEngine(deps)
      await engine.start()
      const pullCrdtForNote = vi.spyOn(
      ...
      while (ctx.inFlightSync) await ctx.inFlightSync.catch(() => {})

      expect(getServerMock).toHaveBeenCalled()
      expect(pullCrdtForNote).not.toHaveBeenCalled()
