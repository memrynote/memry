...
import type { LargeFileWorkerMessage } from './large-file-index-worker-protocol'

const mocks = vi.hoisted(() => ({
  posted: [] as LargeFileWorkerMessage[],
  workerData: {} as unknown
}))

vi.mock('worker_threads', () => ({
  parentPort: {
    postMessage: (message: LargeFileWorkerMessage) => mocks.posted.push(message)
  },
...
/** Import the worker entry, which runs its job as a side effect of loading. */
...
describe('large-file index worker', () => {
  ...
  it('posts the finished index back to the bridge', async () => {
    ...
    try {
      // #when
      await runWorker({ kind: 'index', absolutePath: path, fileBytes: body.length })
  ...
  it('posts the hits of a search job back to the bridge', async () => {
    // #given a file with the query on two of its lines
    const dir = await mkdtemp(join(tmpdir(), 'memry-large-file-worker-'))
    const path = join(dir, 'log.md')
    ...
    try {
      // #when
      await runWorker({ kind: 'search', absolutePath: path, query: 'beta', maxHits: 50 })
      await vi.waitFor(() => expect(mocks.posted.at(-1)?.type).toBe('search-done'))

      // #then — one entry carries both whole-file passes, so a search at 2 GB
      // runs where the scan does and never on the thread that draws the window
      expect(mocks.posted.at(-1)).toMatchObject({
        type: 'search-done',
        hits: [
          { line: 1, ordinal: 0 },
          { line: 2, ordinal: 0 }
  ...
  it('posts an error rather than dying silently', async () => {
    ...
    try {
      // #when
      await runWorker({ kind: 'index', absolutePath: join(dir, 'gone.md'), fileBytes: 10 })
  ...
  it('posts an error when a search job cannot open the file', async () => {
    // #given a file that went away between the open and the search
    const dir = await mkdtemp(join(tmpdir(), 'memry-large-file-worker-'))
    ...
    try {
      ...
      await runWorker({
        kind: 'search',
        absolutePath: join(dir, 'gone.md'),
        query: 'beta',
        maxHits: 50
      })
      await vi.waitFor(() => expect(mocks.posted.at(-1)?.type).toBe('error'))

      // #then — the bridge waits on a message; an unspoken failure is a hang
      expect(mocks.posted.at(-1)).toMatchObject({ type: 'error', message: expect.any(String) })
    } finally {
      await rm(dir, { recursive: true, force: true })
    }
