...
describe('subtask-bulk-utils', () => {
  ...
  describe('T135: completion checks', () => {
    it('returns true when all subtasks are complete', () => {
      ...
      const result = checkAllSubtasksComplete('parent-1', [parent, subtask1, subtask2])
    ...
    it('counts incomplete subtasks', () => {
      ...
      const result = getIncompleteSubtaskCount('parent-1', [parent, subtask1, subtask2, subtask3])
  ...
  describe('T136: bulk completion updates', () => {
    it('completes only incomplete subtasks', () => {
      ...
      const now = new Date()
      const result = completeAllSubtasks('parent-1', [parent, subtask1, subtask2])

      expect(result.success).toBe(true)
      expect(result.affectedCount).toBe(1)
      expect(result.completedSubtasks?.map((task) => task.id)).toEqual(['sub-1'])

      const updatedSubtask1 = result.updatedTasks?.find((task) => task.id === 'sub-1')
      const updatedSubtask2 = result.updatedTasks?.find((task) => task.id === 'sub-2')
    ...
    it('marks only completed subtasks incomplete', () => {
      ...
      const result = markAllSubtasksIncomplete('parent-1', [parent, subtask1, subtask2])

      expect(result.success).toBe(true)
      expect(result.affectedCount).toBe(1)
      expect(result.incompleteSubtasks?.map((task) => task.id)).toEqual(['sub-1'])
  ...
  describe('T137: bulk due date and priority updates', () => {
    it('sets due date for incomplete subtasks when exclude completed', () => {
      ...
      const result = setDueDateForAllSubtasks('parent-1', dueDate, false, [
        parent,
        subtask1,
        subtask2
      ])
    ...
    it('sets due date for all subtasks when include completed', () => {
      ...
      const result = setDueDateForAllSubtasks('parent-1', dueDate, true, [
        parent,
        subtask1,
        subtask2
      ])
    ...
    it('returns error when no matching subtasks for due date update', () => {
      ...
      const result = setDueDateForAllSubtasks('parent-1', new Date(2026, 0, 20), false, [
        parent,
        subtask1
      ])
    ...
    it('sets priority for incomplete subtasks when exclude completed', () => {
      ...
      const result = setPriorityForAllSubtasks('parent-1', 'urgent', false, [
        parent,
        subtask1,
        subtask2
      ])
  ...
  describe('T138: deleteAllSubtasks', () => {
    it('removes all subtasks and clears parent references', () => {
      ...
      const result = deleteAllSubtasks('parent-1', [parent, subtask1, subtask2, otherTask])
  ...
  describe('T139: duplicateTaskWithSubtasks', () => {
    it('duplicates a task and its subtasks with new IDs', () => {
      ...
      const duplicatedSubtasks =
        result.updatedTasks?.filter((task) => task.parentId === duplicatedTask?.id) ?? []

      expect(duplicatedSubtasks).toHaveLength(2)
      expect(duplicatedSubtasks.map((task) => task.title)).toEqual(['Subtask One', 'Subtask Two'])
    ...
    it('duplicates only the task when includeSubtasks is false', () => {
      ...
      const result = duplicateTaskWithSubtasks('parent-1', false, [parent, subtask1])
  ...
  describe('T140: parent completion helpers', () => {
    it('completes the parent task when incomplete', () => {
      ...
      expect(result.success).toBe(true)
      const updatedParent = result.updatedTasks?.find((task) => task.id === 'parent-1')
